...
export const modes: { [key: string]: Mode } = {
    ...
    christmas: {
        name: 'Christmas',
        pickCards: (questions: Question[]) => {
            const drinkIfQuestions = questions.filter((question) => question.tags.includes('drinkIf') && question.tags.includes('christmas')).slice(0, 10);
            const commonQuestions = questions.filter((question) => question.tags.includes('christmas') && !question.tags.includes('drinkIf') && !question.tags.includes('event')).slice(0, 10);
            const eventQuestion = questions.filter((question) => question.tags.includes('event') && question.tags.includes('christmas'))[0];
            let finalQuestions = [...drinkIfQuestions, ...commonQuestions];
